...
class InvestigationService:
    ...
    def get_evidence_library(self, investigation_id: str) -> List[Dict[str, Any]]:
        ...
        clean_id = rec.image_id.replace(".tif", "") if rec.image_id else "unknown"
        output_dir = settings.REPO_ROOT / "demo" / "output"
        ...
        source_tiff = Path(rec.source_image_path) if rec.source_image_path else (settings.REPO_ROOT / "data" / "sentinel_images" / f"{clean_id}.tif")
        mask_png = output_dir / f"real_{clean_id}_mask.png"
        mask_tif = output_dir / f"real_{clean_id}_mask.tif"
        layers_geojson = output_dir / f"real_{clean_id}_layers.geojson"
        drift_csv = output_dir / f"real_{clean_id}_drift_trajectory.csv"
        drift_json = output_dir / f"real_{clean_id}_drift_trajectory.json"
        result_json = output_dir / f"real_{clean_id}_result.json"

        st_info = _file_info(source_tiff)
        mp_info = _file_info(mask_png)
        mt_info = _file_info(mask_tif)
        lg_info = _file_info(layers_geojson)
        dc_info = _file_info(drift_csv)
        dj_info = _file_info(drift_json)
        rj_info = _file_info(result_json)

        artifacts = [
            {
                "name": "Sentinel-1 SAR C-Band GeoTIFF",
                "category": "SATELLITE",
                "status": "AVAILABLE" if st_info["exists"] else "UNAVAILABLE",
                "file_name": source_tiff.name if st_info["exists"] else f"{clean_id}.tif",
                "file_path": str(source_tiff) if st_info["exists"] else None,
                "file_size_bytes": st_info["size_bytes"],
                "generated_at": st_info["mtime"] or (rec.observation_timestamp.isoformat() if rec.observation_timestamp else None),
                "provenance_source": "Copernicus Open Access Hub / ESA Sentinel-1",
                "download_url": f"/api/images/{clean_id}/file" if st_info["exists"] else None,
                "preview_type": "none",
            },
            {
                "name": "U-Net Oil Slick Segmentation Mask (PNG)",
                "category": "SEGMENTATION",
                "status": "AVAILABLE" if mp_info["exists"] else "UNAVAILABLE",
                "file_name": mask_png.name,
                "file_path": str(mask_png) if mp_info["exists"] else None,
                "file_size_bytes": mp_info["size_bytes"],
                "generated_at": mp_info["mtime"],
                "provenance_source": "OILTRACE U-Net ResNet34 Inference (unet_best.pth)",
                "download_url": None,
                "preview_type": "image" if mp_info["exists"] else "none",
            },
            {
                "name": "Georeferenced Oil Slick GeoTIFF",
                "category": "SEGMENTATION",
                "status": "AVAILABLE" if mt_info["exists"] else "UNAVAILABLE",
                "file_name": mask_tif.name,
                "file_path": str(mask_tif) if mt_info["exists"] else None,
                "file_size_bytes": mt_info["size_bytes"],
                "generated_at": mt_info["mtime"],
                "provenance_source": "Rasterio Spatial Affine Transform (EPSG:4326)",
                "download_url": None,
                "preview_type": "none",
            },
            {
                "name": "Multi-Layer GIS GeoJSON FeatureCollection",
                "category": "GIS",
                "status": "AVAILABLE" if (lg_info["exists"] or rec.geojson_layers) else "UNAVAILABLE",
                "file_name": layers_geojson.name,
                "file_path": str(layers_geojson) if lg_info["exists"] else None,
                "file_size_bytes": lg_info["size_bytes"] or len(str(rec.geojson_layers or "")),
                "generated_at": lg_info["mtime"],
                "provenance_source": "Shapely / Geodesic Polygon Vectorizer",
                "download_url": f"/api/investigations/{investigation_id}/map",
                "preview_type": "geojson",
            },
            {
                "name": "Lagrangian Ocean Drift Advection Coordinates (CSV)",
                "category": "OCEAN_DRIFT",
                "status": "AVAILABLE" if dc_info["exists"] else "UNAVAILABLE",
                "file_name": drift_csv.name,
                "file_path": str(drift_csv) if dc_info["exists"] else None,
                "file_size_bytes": dc_info["size_bytes"],
                "generated_at": dc_info["mtime"],
                "provenance_source": "Copernicus CMEMS Surface Currents (Euler Hindcast)",
                "download_url": None,
                "preview_type": "table",
            },
            {
                "name": "Drift Trajectory Time-Stepped Vectors (JSON)",
                "category": "OCEAN_DRIFT",
                "status": "AVAILABLE" if dj_info["exists"] else "UNAVAILABLE",
                "file_name": drift_json.name,
                "file_path": str(drift_json) if dj_info["exists"] else None,
                "file_size_bytes": dj_info["size_bytes"],
                "generated_at": dj_info["mtime"],
                "provenance_source": "Lagrangian Advection RK4 / Euler Predictor",
                "download_url": f"/api/investigations/{investigation_id}/drift",
                "preview_type": "json",
            },
            {
                "name": "AIS Vessel Candidate Attribution Matrix",
                "category": "AIS_ATTRIBUTION",
                "status": "AVAILABLE" if rec.result_json else "UNAVAILABLE",
                "file_name": f"ais_attribution_{investigation_id}.json",
                "file_path": None,
                "file_size_bytes": len(str(rec.result_json or "")),
                "generated_at": rec.updated_at.isoformat() if rec.updated_at else None,
                "provenance_source": "Global Fishing Watch 4Wings AIS API / Space-Time Ranker",
                "download_url": f"/api/investigations/{investigation_id}/vessels",
                "preview_type": "table",
            },
            {
                "name": "IMO MARPOL 73/78 Annex I Legal Evidence Dossier",
                "category": "LEGAL_REPORT",
                "status": "AVAILABLE" if rec.result_json else "UNAVAILABLE",
                "file_name": f"OILTRACE_Report_{investigation_id}.md",
                "file_path": None,
                "file_size_bytes": 14500 if rec.result_json else 0,
                "generated_at": rec.updated_at.isoformat() if rec.updated_at else None,
                "provenance_source": "OILTRACE Automated Forensic Report Generator",
                "download_url": f"/api/investigations/{investigation_id}/report/download",
                "preview_type": "text",
            },
        ]
        return artifacts
