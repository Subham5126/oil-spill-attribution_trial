...
import { Eye, EyeOff, CheckCircle2, XCircle, AlertCircle, Loader2, ArrowRight, ShieldCheck } from "lucide-react";
import { verifyActivationToken, activateAccount, TokenVerificationData } from "../services/api";

interface ActivateAccountPageProps {
  onSuccess?: () => void;
}

export function ActivateAccountPage({ onSuccess }: ActivateAccountPageProps) {
  ...
  useEffect(() => {
    ...
    if (!tok) {
      setVerifying(false);
      setVerifyError("No activation token found. Please use the complete link from your invitation email.");
      return;
    }
    setToken(tok);

    verifyActivationToken(tok)
      .then((info) => {
        setEmployeeInfo(info);
        setVerifying(false);
      })
      .catch((err: any) => {
        setVerifyError(err?.message || "This activation link is invalid or has expired. Please contact your system administrator.");
        setVerifying(false);
      });
  }, []);
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err?.message || "Failed to activate account. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="bg-slate-900/90 border border-slate-700/60 rounded-2xl p-8 shadow-2xl shadow-black/50 space-y-6 backdrop-blur-sm">
          ...
          <div className="flex flex-col items-center text-center space-y-2">
            <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center shadow-lg mb-1">
              <span className="material-symbols-outlined text-[32px] text-cyan-400">shield_person</span>
            </div>
          ...
          {verifying && (
            <div className="flex flex-col items-center justify-center py-10 space-y-3">
              ...
              <p className="text-xs font-mono text-slate-400">Verifying activation invitation...</p>
            </div>
          )}
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-3 p-4 rounded-xl bg-red-950/40 border border-red-800/50 text-red-200">
                ...
                <div className="text-xs font-mono space-y-1">
                  <p className="font-semibold text-red-300">Activation Link Invalid or Expired</p>
                  <p className="text-slate-400 text-[11px]">{verifyError}</p>
                </div>
              </div>
              <p className="text-xs text-slate-400 font-sans text-center">
                Please contact your Tech Administrator to receive a newly issued activation invitation link.
              </p>
              <button
                type="button"
            ...
            <div className="space-y-6 text-center py-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-300 font-sans">
                  Your credentials have been securely registered with Coast Authority. You may now sign in.
                </p>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  {employeeInfo.department && (
                    <div className="col-span-2">
                      ...
                      <span className="text-slate-300">{employeeInfo.department}</span>
                    </div>
                  )}
                </div>
              </div>
